...
/**
 * Class SecurityCodeCreateAction
 * 创建验证码
 * @package app\src\securitycode\action
 */
class SecurityCodeCreateAction extends BaseAction
{
    // 中文验证码字符串
     ...
     * @param string $acceptor 接收人
     * @param string $type 类型
     * @param string $code_create_way 验证码创建方式
     * @param int $code_length  验证码长度 3 < length < 8
     * @param int $expire_time 过期时间，默认1800秒
     * @return array
     */
    public function create($client_id,$acceptor,$type,$code_create_way="only_number",$code_length=6,$expire_time = 1800){

        if($code_length > 8 || $code_length < 3){
            $code_length = 6;
        }
        ...
        //生成验证码6位
        if($code_create_way == CodeCreateWayEnum::ONLY_NUMBER){
            $code = $this->randNumberCode($code_length);
        }elseif($code_create_way == CodeCreateWayEnum::ALPHA_AND_NUMBER){
            $code = $this->randCode($code_length);
        }else{
            $code = $this->randNumberCode($code_length);
        }
        ...
        //1. 重置该手机号对应的验证码
        $securityCodeLogic->resetAll($acceptor,$type,$client_id);
     ...
     * @return int
     */
    private function randNumberCode($length){
     ...
     * @return array
     */
    private function randCode($length){
