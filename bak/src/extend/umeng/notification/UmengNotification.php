...
// addLog('post_data',$this->data,$url,'send_before');
        ...
        if ($httpCode == "0") {
			...
			require_once __DIR__ . '/../UmengErrorCode.php';

			$result = json_decode($result,true);
			$errorInfo = UmengErrorCode::getError(strval($result['data']['error_code']));
			throw new Exception("Http code:" . $httpCode .  " details:" . $result['ret'] .  " error_code:" . $result['data']['error_code'] . $errorInfo .  "\r\n");
        } else {
           	return $result;
        }
    }
