...
use app\src\system\helper\DbHelper;
use think\Db;
...
class Database extends Admin
{
    ...
    public function index($type = null){
        ...
        return $this->boye_display("default",'Database/'.$type);
    }
    ...
    public function export($tables = null, $id = null, $start = null){
        if(IS_POST && !empty($tables) && is_array($tables)){ //初始化
            ...
            //创建备份文件
            $Database = new DbHelper($file, $config);
            if(false !== $Database->create()){
                $tab = array('id' => 0, 'start' => 0);
                $this->success('初始化成功！', url('Database/export'), array('tables' => $tables, 'tab' => $tab));
            } else {
            ...
            //备份指定表
            $Database = new DbHelper(session('backup_file'), session('backup_config'));
            ...
            if(false === $start){ //出错
                ...
                $this->success("正在备份...({$rate}%)", url('Database/export'), array('tab' => $tab));
            }
