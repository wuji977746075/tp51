...
use src\hook\logic\HookLogic;

class Addons extends CheckLogin {
//     public function _initialize(){
//         $this->assign('_extra_menu',array(
//             '已装插件后台'=> apiCall(AddonsApi::GET_ADMIN_LIST),
//         ));
//         parent::_initialize();
//     }
...
// //        $result = apiCall(HooksApi::QUERY_NO_PAGING,array(array(),false,$fields));
//         $result = (new HookLogic())->queryNoPaging([],false,$fields);
// //        dump($result);
//         if(!$result['status']){
//             $this->error($result['info']);
//         }

//         $this->assign('Hooks',$result['info']);
//         $this->assignTitle("创建向导");
...
//         $this->assignTitle("插件列表");
//         $map = array();
//         $page = array('curpage'=>$this->_param('p',0),'size'=>10);
//         $list       =  apiCall(AddonsApi::GET_LIST);


// //        $list = int_to_string($list['info']['list'],"status",array(-1=>'损坏', 0=>'禁用', 1=>'启用', null=>'未安装'));

//         $this->assign('_list', $list);
//         $this->assign('_page', $list['info']['show']);
//         // 记录当前列表页的cookie
...
//             $fetchContent = $this->fetch($addon->addon_path.$addon->custom_adminlist);
//             $this->assign('custom_adminlist', $fetchContent);
//         }
//         return $this->boye_display('adminlist');
//     }
...
//         $options    =   array();
//         $REQUEST    =   (array)$this->_param('request.');
//         if(is_string($model)){
//             $model  =   M($model);
//         }
...
//         $id     =   $this->_param('id');
//         $result = apiCall(AddonsApi::ENABLE,array(array("id"=>$id)));
//         if(!$result['status']){
//             $this->error($result['info']);
//         }
//         cache('global_hooks', null);
//         $this->success("启用成功!");
//     }
...
//         $id     =   $this->_param('id');
//         $result = apiCall(AddonsApi::DISABLE,array(array("id"=>$id)));
//         if(!$result['status']){
//             $this->error($result['info']);
//         }
...
//         $addon  =   M('Addons','common_')->find($id);
//         if(!$addon)
//             $this->error('插件未安装');
...
//         $id     =   (int)$this->_param('id');
//         $config =   $this->_param('config');

//         $result = apiCall(AddonsApi::SAVE_BY_ID,array($id,array('config'=>json_encode($config))));
...
//         $hook_mod =Db::table('common_Hooks');
//         $where['name'] = $str;
...
//      */
//     public function install(){
//         $addon_name     =   trim($this->_param('addon_name'));
...
//             $hooks_update   =   apiCall(HooksApi::UPDATE_HOOKS,array($addon_name));
//             if($hooks_update['status']){
//                 cache('hooks', null);
//                 $this->success('安装成功');
//             }else{
...
//      */
//     public function uninstall(){
//         $id             =   trim($this->_param('id'));
//         $db_addons    =   apiCall(AddonsApi::GET_INFO,array(array('id'=>$id)));
// //        dump($db_addons);
//         if(!$db_addons['status']){
//             $this->error($db_addons['info']);
//         }

//         $db_addons      =   $db_addons['info'];
...
//         $result = apiCall(HooksApi::REMOVE_HOOKS,array($db_addons['name']));
//         if(!$result['status']){
//             $this->error($result['info']);
//         }

//         $hooks_update   = $result['info'];  // D('Hooks')->removeHooks($db_addons['name']);
...
//         cache('global_hooks', null);
//         $result = apiCall(AddonsApi::DELETE,array(array('name'=>$db_addons['name'])));
...
//         $this->assignTitle('钩子列表');
//         $map    =   $fields =   array();
//         $page   =   array('curpage'=>$this->_param('p'),'size'=>10);
//         $order  =   'update_time desc';
//         $result =   apiCall(HooksApi::QUERY,array($map,$page,$order));
//         if(!$result['status']){
//             $this->error($result['info']);
//         }
...
//         $this->assignTitle('新增钩子');
//         return $this->boye_display('edithook');
//     }

//     //钩子出编辑挂载插件页面
//     public function edithook($id){
//         $result = apiCall(HooksApi::GET_INFO,array(array("id"=>$id)));
//         if(!$result['status']){
//             $this->error($result['info']);
//         }
//         $this->assign('data',$result['info']);
//         $this->assignTitle('编辑钩子');
//         return $this->boye_display('edithook');
//     }

//     //超级管理员删除钩子
//     public function delhook($id){
//         $result = apiCall(HooksApi::DELETE,array(array("id"=>$id)));
//         if($result['status']){
//             $this->success('删除成功');
//         }else{
//             $this->error('删除失败');
...
//         $TMPL_PARSE_STRING['__ADDONROOT__'] = ADDON_PATH."/{$_addons}";
...
//         $ids = array_unique((array)$this->_param('ids',0));
