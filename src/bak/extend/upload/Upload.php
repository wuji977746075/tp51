...
// +----------------------------------------------------------------------
namespace app\src\extend\upload;
//use app\src\system\api\UserPictureApi;
//curl_upload use only
...
class Upload {
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            if(false === $subpath){
                ...
                $file['savepath'] = $this->rootPath.$this->savePath.$subpath;
            }
    ...
    public function curl_upload($files=''){
...
//     [1] => string(3) \"Ymd\"\n
// }\n
// [\"rootPath\"] => string(21) \".\/upload\/userPicture\/\"\n
// [\"savePath\"] => string(6) \"other\/\"\n
// [\"saveName\"] => array(2) {\n
//     [0] => string(6) \"uniqid\"\n
//     [1] => string(0) \"\"\n
// }\n
...
// [\"driverConfig\"] => array(0) {\n  }\n
        ...
        /* 上传文件 */
        $file =[];
        $file['name'] = strip_tags($files['name']);
        $file['type'] = $files['type'];
        $file['size'] = strlen($files['data']);
        $file['ext']  = 'png';//TODO

        /* 获取文件hash */
        if($this->hash){
            $file['md5']  = md5($files['data']);
            $file['sha1'] = sha1($files['data']);
        }
        /* 检测文件 md5/sha1是否存在 */
        // $map = ' `md5`=\''.$file['md5'].'\' or `sha1`=\''.$file['sha1'].'\' ';
        ...
        if(file_put_contents($filename, $files['data'])){
            return $file;
        }else{
           $this->error = '文件写入失败！';
           return false;
    ...
    private function setDriver($driver = null, $config = null){
        $driver = $driver ? : ($this->driver       ? : config('FILE_UPLOAD_TYPE'));
        $config = $config ? : ($this->driverConfig ? : config('UPLOAD_TYPE_CONFIG'));
        $class = strpos($driver,'\\')? $driver : 'app\\src\\extend\\upload\\driver\\'.ucfirst(strtolower($driver));
        $this->uploader = new $class($config);
        if(!$this->uploader){
            throw new Exception("不存在上传驱动：{$class}", 100006);
        }
    }
