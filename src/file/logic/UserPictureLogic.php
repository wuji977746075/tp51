...
class UserPictureLogic extends BaseLogic
{
    ...
    public function upload($files, $setting,$extInfo, $driver = 'local', $config = null){
        ...
        foreach($files as $file){
            ...
            $r = sdb('itboye_user_picture','')->where(['md5'=>$md5,'status'=>1,'ori_name'=>$name])->field($field)->find();
            if(!empty($r)){ //无需记录
                //todo : 真实图片是否存在
                // $path = '.'.$field['path'];
                // if($is_file($path)){
                ...
                // }
                $img_info = $r;
                $r2 = sdb('itboye_user_picture','')->where(['md5'=>$md5,'uid'=>$uid,'type'=>$type,'status'=>1,'ori_name'=>$name])->field($field)->find();
                if(empty($r2)){
                    //该图片该类型该用户未上传过
                    unset($r['id']);
                    $r['uid']      = $uid;
